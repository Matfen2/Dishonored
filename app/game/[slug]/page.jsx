...
const GamePage = () => {
  ...
  // Déplacez les hooks avant tout retour conditionnel
  const [selectedImage, setSelectedImage] = useState(game ? game.wallpaper[0] : null);

  // Si le jeu n'est pas trouvé, afficher un message
  if (!game) {
    return <div className="text-center text-lg mt-10">Jeu non trouvé</div>;
  }
  ...
  return (
    <div className="game">
      ...
      <div
        ...
        style={{ backgroundImage: `url(${game.imgGame})` }}
      >
        <Link href="/">
          <div
            className="fixed top-4 left-0 z-20"
            style={{ marginLeft: "-72px", marginTop: "-35px" }}
          >
            <Image
              src="/picts/dishonoredLogo.png"
              alt="logoDishonored"
              width={320}
              height={120}
        ...
        <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex space-x-4 text-center">
          <a
            ...
            className="px-3 py-2 border border-white bg-white text-black text-sm rounded-md transition-transform duration-200 hover:scale-105"
            id="hrefBuy"
          >
            GAMEPLAY
          </a>
      ...
      <div className="bg-black p-6 md:p-10 grid grid-cols-1 md:grid-cols-3 gap-4 text-white">
        {game.jacket.imgJacket.map((img, index) => (
          <div key={index} className="text-center px-4 md:py-4">
            ...
            />
            <h3 className="text-lg font-semibold mt-4 mb-2 md:mb-1" id="pitchJacket">
              {game.jacket.pitchJacket[index]}
            </h3>
            <p
              className="text-sm md:max-w-xs mx-auto leading-relaxed"
              id="sentenceJacket"
            >
              {game.jacket.sentenceJacket[index]}
            </p>
          </div>
        ))}
      </div>
        ...
        <div className="relative z-10 flex flex-col justify-center md:flex-row items-center space-x-0 md:space-x-6 lg:space-x-8 w-full">
          ...
          <div className="flex flex-col max-w-2xl text-center md:text-left px-4 md:px-0">
            ...
            <div className="mt-4 flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 justify-center md:justify-start">
              <a
                href={game.buy.hrefBuy.playstation}
                target="_blank"
                rel="noopener noreferrer"
              >
                <button className="px-4 py-2 text-white rounded-md transition-transform duration-200 hover:scale-105" id="btnPlaystation">
                  PLAYSTATION
                </button>
              </a>
              <a href={game.buy.hrefBuy.xbox} target="_blank" rel="noopener noreferrer">
                <button className="px-4 py-2 text-white rounded-md transition-transform duration-200 hover:scale-105" id="btnXbox">
                  XBOX
                </button>
              </a>
              <a href={game.buy.hrefBuy.steam} target="_blank" rel="noopener noreferrer">
                <button className="px-4 py-2 text-white rounded-md transition-transform duration-200 hover:scale-105" id="btnSteam">
                  STEAM
                </button>
              </a>
            </div>
